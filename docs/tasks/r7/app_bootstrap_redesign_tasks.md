...
| 6 | Data step | `core/bootstrap/bootstrap_service.dart` | Preload categories + member profiles cho account hiện tại (family). Gọi `FeedHelper.preloadNames(account.memberIds)` để cache sẵn tên + avatar trước khi vào HomeScreen. Fail → empty, vẫn vào app |
...
| 10 | L10n keys | `app_vi.dart`, `app_en.dart` | Thêm: `bootstrapAuth` (Đang kết nối.../Connecting...), `bootstrapAccount` (Đang mở sổ.../Opening ledger...), `bootstrapSettings` (Đang tải cài đặt.../Loading settings...), `bootstrapData` (Đang tải dữ liệu.../Loading data...), `bootstrapAlmost` (Sắp xong rồi.../Almost ready...) |
