...
class WalletService {
  ...
  Future<String> createWallet(String name, int initialBalance, {String currency = 'VND', WalletType type = WalletType.normal}) async {
  ...
  Future<void> updateWallet(String id, String name, {String? currency, int? initialBalance, WalletType? type}) async {
    ...
    if (initialBalance != null) {
      ...
      // Also persist other changed fields (name, currency, type)
      data.remove('initial_balance');
      if (data.isNotEmpty) await _repo.update(id, data);
      return;
    }
